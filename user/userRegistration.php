...
if(isset($_POST['userRegister'])){
  ...
  if(mysqli_num_rows($selectResult)==0){
    ...
    $sql="insert into client(firstname,middlename,lastname,username,password,buildingid) values('$userFname','$userMname','$userLname','$username','$hashedPassword',$building)";
...
<html lang="en">
    ...
    <select name="building" id="building">
        ...
        if($result){
            while($row=mysqli_fetch_assoc($result)){
                echo '<option value="'.$row['buildingId'].'">'.$row['location'].'</option>';
  ...
  <button type="submit" name="userRegister" class="btn btn-primary w-100">Submit</button>
  <p class="small mt-3">already have account?<a href="userLogin.php" class="text-decoration-none mt-3">Login</a></p>
  <p><?php echo $alert ?></p>
</form>
